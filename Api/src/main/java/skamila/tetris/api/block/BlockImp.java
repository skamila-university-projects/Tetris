...
public class BlockImp implements Block {
	...
	public void rotate(Board board) {
		...
		for (i = 0; i < points.length; i++) {
			if (points[i].getY() + shiftVertical >= board.getHeight() - 1)
				return;
			if (points[i].getY() + shiftVertical > 0 && board
					.getField(points[i].getX() + newShiftHorizontal, points[i].getY() + shiftVertical).isOccupied())
				return;
		}
	...
	private boolean isUnderOccupied(Board board, StatePoint statePoint) {
		...
		return board.getField(statePoint.getX() + shiftHorizontal, statePoint.getY() + shiftVertical + 1).isOccupied();
	}
	...
	private boolean isLeftOccupied(Board board, StatePoint statePoint) {
		...
		return board.getField(statePoint.getX() + shiftHorizontal - 1, statePoint.getY() + shiftVertical).isOccupied();
	}
	...
	public BlockState getShiftedActiveState() {
		...
		for (int i = 0; i < statePoints.length; i++) {
			shiftedStatePoints[i] = new StatePoint(statePoints[i].getX() + shiftHorizontal,
